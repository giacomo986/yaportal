...
// set_pos_look(self, pos, yaw_radians)
// Atomically sets position and horizontal look angle in a single MOVE_PLAYER packet.
int ObjectRef::l_set_pos_look(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkObject<ObjectRef>(L, 1);
	ServerActiveObject *sao = getobject(ref);
	...
	// Set yaw first (no packet sent) so the subsequent setPos() sends a single
	// MOVE_PLAYER packet carrying both the new position and the new yaw.
...
luaL_Reg ObjectRef::methods[] = {
	...
	luamethod(ObjectRef, set_pos_look),
	luamethod(ObjectRef, portal_teleport),
