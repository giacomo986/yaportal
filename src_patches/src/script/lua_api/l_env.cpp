...
// set_portals(list)
// list: Lua array of {pos, normal, up, half_w, half_h, link} tables (1-based).
// pos in node coordinates (multiplied by BS internally).
// half_w/half_h in node coordinates (half-extents of the portal opening).
// link: 0-based index of the destination portal (-1 or absent = no link).
int ModApiEnv::l_set_portals(lua_State *L)
{
	...
	int n = std::min((int)lua_objlen(L, 1), MAX_PORTALS);

	// Set active portals first so the render thread never sees a fully-empty state.
	for (int i = 1; i <= n; ++i) {
		lua_rawgeti(L, 1, i);
		if (!lua_istable(L, -1)) { lua_pop(L, 1); pm.clearPortal(i - 1); continue; }
		...
		lua_getfield(L, -1, "half_w");
		float half_w = lua_isnumber(L, -1) ? (float)lua_tonumber(L, -1) * BS : BS;
		lua_pop(L, 1);

		lua_getfield(L, -1, "half_h");
		float half_h = lua_isnumber(L, -1) ? (float)lua_tonumber(L, -1) * BS : 1.5f * BS;
		lua_pop(L, 1);

		lua_getfield(L, -1, "link");
		int link = lua_isnumber(L, -1) ? (int)lua_tonumber(L, -1) : -1;
		lua_pop(L, 1);

		normal.normalize();
		up.normalize();
		pm.setPortal(i - 1, pos, normal, up, half_w, half_h, link);
		lua_pop(L, 1);
	}
	// Clear slots beyond the list.
	for (int i = n; i < MAX_PORTALS; ++i)
		pm.clearPortal(i);
	return 0;
}

// set_portal_cam_hint(portal_id, pos)
// portal_id: 0=blue, 1=orange. pos in node coordinates.
// Sets a persistent camera position override for RTT renders of that portal.
// Hint remains valid until clear_portal_cam_hint is called.
int ModApiEnv::l_set_portal_cam_hint(lua_State *L)
{
	int id = luaL_checkint(L, 1);
	if (id < 0 || id >= MAX_PORTALS)
		return 0;
	v3f pos = read_v3f(L, 2) * BS;
	PortalManager::get().setCamHint(id, pos);
	return 0;
}

// clear_portal_cam_hint(portal_id)
// Removes the camera position override so the RTT reverts to following mainCam.
int ModApiEnv::l_clear_portal_cam_hint(lua_State *L)
{
	int id = luaL_checkint(L, 1);
	if (id < 0 || id >= MAX_PORTALS)
		return 0;
	PortalManager::get().clearCamHint(id);
	return 0;
}

// set_portal_sky(portal_id, {visible=bool, clear_color="#rrggbb"})
// Overrides the sky rendered inside the portal's RTT.
// visible=false: sky draws nothing; clear_color becomes the RTT background.
int ModApiEnv::l_set_portal_sky(lua_State *L)
{
	...
	if (lua_istable(L, 2)) {
		...
		if (lua_isstring(L, -1)) {
			video::SColor c(0xFF000000);
			parseColorString(lua_tostring(L, -1), c, false, 0xFF);
			argb = c.color;
		}
		lua_pop(L, 1);
	}
	PortalManager::get().setSkyConfig(id, visible, argb);
	return 0;
}
