...
queue_full_break:
	...
	// Send/emerge blocks around portal destinations so the virtual camera
	// has terrain to render when portals are far from the player.
	{
		const PortalManager &pm = PortalManager::get();
		if (pm.anyActive()) {
			const s16 portal_d_max = std::min<s16>(full_d_max, 4);
			for (int pi = 0; pi < MAX_PORTALS; pi++) {
				if (!pm.portal(pi).active) continue;
				v3s16 pcenter = getNodeBlockPos(
					floatToInt(pm.portal(pi).pos, BS));
				for (s16 pd = 0; pd <= portal_d_max; pd++) {
					for (const v3s16 &p_rel : FacePositionCache::getFacePositions(pd)) {
						v3s16 p = p_rel + pcenter;
						if (blockpos_over_max_limit(p)) continue;
						if (m_blocks_sending.find(p) != m_blocks_sending.end()) continue;
						if (m_blocks_sent.find(p) != m_blocks_sent.end()) continue;
						if (num_blocks_selected >= m_max_simul_sends) goto portal_send_done;
						MapBlock *block = env->getMap().getBlockNoCreateNoEx(p);
						if (!block || !block->isGenerated()) {
							emerge->enqueueBlockEmerge(peer_id, p, true);
							continue;
						}
						block->resetUsageTimer();
						dest.emplace_back(1000.0f + (float)pd, p, peer_id);
						num_blocks_selected++;
					}
				}
			}
