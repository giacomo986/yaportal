...
std::unique_ptr<RenderStep> create3DStage(Client *client, v2f scale)
{
	...
	if (g_settings->getBool("enable_post_processing")) {
		...
		// Portal split: prepare (RTT) BEFORE Draw3D so the RTT render leaves
		// FBO=0/screen. Draw3D's m_target->activate() then cleanly re-binds the
		// TextureBuffer FBO via Irrlicht's cache handler (no GL.BindFramebuffer hack).
		// Quad draw AFTER Draw3D uses the intact 3D depth buffer, BEFORE post-proc.
		auto portal_data = std::make_shared<PortalRTTData>();
		...
		// Give PortalQuadStep access to Draw3D's render target so it can restore
		// it after re-rendering RTTs on an intra-frame teleport.
		if (auto *d3d = dynamic_cast<Draw3D*>(step))
			portal_data->scene_rt = d3d->getRenderTarget();
