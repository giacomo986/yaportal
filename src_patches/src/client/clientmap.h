...
public:
	...
	*/
	...
	// Portal RTT support: build/clear an extra drawlist from virtual camera positions.
	// Blocks in this list are rendered alongside m_drawlist without frustum culling.
	// Call updatePortalDrawList before portal smgr->drawAll(), clearPortalDrawList after.
	void updatePortalDrawList(const std::vector<v3f> &portal_cam_positions);
	void clearPortalDrawList();

	void renderMap(video::IVideoDriver* driver, s32 pass);

	// Skip frustum culling in renderMap() — used by portal RTT pass so the
...
private:
	...
	// Extra drawlist for portal RTT pass: blocks near virtual cameras
	std::map<v3s16, MapBlock*> m_portal_drawlist;
	std::vector<MapBlock*> m_portal_keeplist;
