		...
		std::vector<CollisionInfo> *collision_info)
{
	...
	*/
	...
	// Raise box bottom for floor-portal passthrough (see physics_override.floor_portal_cb_shift).
	aabb3f phys_box = m_collisionbox;
	if (physics_override.floor_portal_cb_shift != 0.0f) {
		float shift = physics_override.floor_portal_cb_shift * BS;
		// Clamp so MinEdge.Y never exceeds MaxEdge.Y (degenerate box).
		shift = std::min(shift, m_collisionbox.MaxEdge.Y - m_collisionbox.MinEdge.Y - 0.01f * BS);
		if (shift > 0.0f)
			phys_box.MinEdge.Y += shift;
	}

	collisionMoveResult result = collisionMoveSimple(env, m_client,
		phys_box, player_stepheight, dtime,
	...
	std::vector<CollisionInfo> *collision_info)
{
	...
	*/
	...
	aabb3f phys_box_old = m_collisionbox;
	if (physics_override.floor_portal_cb_shift != 0.0f)
		phys_box_old.MinEdge.Y += physics_override.floor_portal_cb_shift * BS;

	collisionMoveResult result = collisionMoveSimple(env, m_client,
		phys_box_old, player_stepheight, dtime,
		&position, &m_speed, accel_f, m_cao, true, step_up_mode);
