...
{
	for (auto &i : m_portal_drawlist)
		i.second->refDrop();
	m_portal_drawlist.clear();

	for (auto *block : m_portal_keeplist)
		block->refDrop();
	m_portal_keeplist.clear();
}

void ClientMap::updatePortalDrawList(const std::vector<v3f> &portal_cam_positions)
{
	clearPortalDrawList();
	if (portal_cam_positions.empty())
		return;
	...
	for (auto &sector_it : m_sectors) {
		...
		for (const auto &entry : sector->getBlocks()) {
			...
			bool in_range = false;
			for (const v3f &cam_pos : portal_cam_positions) {
				if (mesh_sphere_center.getDistanceFrom(cam_pos) <=
			...
			// Skip if already covered by the main drawlist
			if (m_drawlist.count(mesh_pos))
				continue;

			if (mesh_grid.cell_size > 1) {
				if (!m_portal_drawlist.count(mesh_pos)) {
				...
				if (!mesh_grid.isMeshPos(bpos) || !block->mesh) {
					block->refGrab();
					m_portal_keeplist.push_back(block);
				}
			} else {
...
void ClientMap::renderMap(video::IVideoDriver* driver, s32 pass)
{
	...
	*/
	...
	for (auto &i : m_drawlist) {
		...
		// Skip when m_bypass_frustum_culling is set (portal RTT uses virtual camera).
		v3f mesh_sphere_center = intToFloat(block->getPosRelative(), BS)
				+ block_mesh->getBoundingSphereCenter();
		f32 mesh_sphere_radius = block_mesh->getBoundingRadius();
		if (!m_bypass_frustum_culling &&
	...
	// Render portal drawlist: blocks near virtual cameras not in the main drawlist.
	// Used during portal RTT passes; empty during the main render.
	for (auto &i : m_portal_drawlist) {
		const v3s16 block_pos = i.first;
		MapBlock *block = i.second;
		MapBlockMesh *block_mesh = block->mesh;
