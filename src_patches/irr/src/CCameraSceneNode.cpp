...
void CCameraSceneNode::setRenderProjectionMatrix(const core::matrix4 &proj)
{
	RenderProj = proj;
	HasRenderProj = true;
}

void CCameraSceneNode::clearRenderProjectionMatrix()
{
	HasRenderProj = false;
}
...
{
	// updateMatrices() computes frustum from ViewArea.ETS_PROJECTION (standard near).
	// HasRenderProj lets us use a different matrix for the driver/shader without
	// skewing the CPU-side frustum used for entity culling.
	updateMatrices();

	video::IVideoDriver *driver = SceneManager->getVideoDriver();
	if (driver) {
		const core::matrix4 &proj = HasRenderProj
			? RenderProj
